...
export default function UserReports() {
  ...
  const [metaUdi, setMetaUdi] = useState<number | null>(0);
  const [metaGyn, setMetaGyn] = useState<number | null>(0);
  const diasUteisNoMes = 21;
  const [diasConcluidos, setDiasConcluidos] = useState<number | null>(null);
  ...
  useEffect(() => {
    // Fazer a solicitação HTTP inicial para obter as configurações
    axios.get('http://localhost:3333/api/config')
      .then(response => {
        const { diasConcluidos, metaGyn, metaUdi } = response.data;
        setDiasConcluidos(diasConcluidos);
        setMetaGyn(metaGyn);
        setMetaUdi(metaUdi);
      })
      .catch(error => {
    ...
    // Estabelecer a conexão SSE apenas uma vez
    if (!sseEstablished) {
      const sse = new EventSource('http://localhost:3333/api/config/sse');
      sse.onmessage = (event) => {
        const config = JSON.parse(event.data);
        setDiasConcluidos(config.diasConcluidos);
        setMetaGyn(config.metaGyn);
        setMetaUdi(config.metaUdi)
      };
